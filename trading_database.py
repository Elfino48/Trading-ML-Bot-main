...
class TradingDatabase:
    ...
    def _init_database(self):
        try:
            ...
            # self._add_new_columns() # This call is removed
    ...
    def store_trade(self, trade_data: Dict[str, Any]) -> bool:
            try:
                ...
                ml_details = trade_data.get('ml_prediction')
                ...
                db_technical_indicators_json = json.dumps(technical_indicators, cls=NpEncoder) if technical_indicators else None
                ...
                data_tuple = (
                    ...
                    db_ml_prediction_details,
                    db_technical_indicators_json
                )
                ...
                sql = '''
                    INSERT INTO trades (
                        ...
                        ml_prediction_details, technical_indicators_json
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                '''
    ...
    def store_performance_metrics(self, metrics: Dict[str, Any]) -> bool:
        try:
            ...
            self.connection.commit()
            self.logger.info(f"Stored performance metrics for {timestamp_str}")
            return True
    ...
    def get_first_trade_timestamp_iso(self) -> Optional[str]:
        """
        Retrieves the timestamp of the very first trade recorded by the bot.
        """
        try:
            query = """
                SELECT MIN(timestamp) as first_trade_time FROM trades
            """
            self.cursor.execute(query)
            row = self.cursor.fetchone()
            if row and row['first_trade_time']:
                self.logger.info(f"Retrieved first trade time: {row['first_trade_time']}")
                return row['first_trade_time']
            else:
                self.logger.warning("Could not find first trade timestamp in database.")
                return None
        except Exception as e:
            self.logger.error(f"Error getting first trade time: {e}", exc_info=True)
            return None
    ...
    def store_prediction_quality(self, symbol: str, prediction: int, actual: int = None,
        ...
        try:
            ...
            if actual is not None:
                db_correct = str(int(prediction) == int(actual))
    ...
    def get_ml_model_performance(self, symbol: str = None, limit: int = 50) -> list:
        try:
            ...
            performance = [dict(row) for row in results]
            return performance
        except Exception as e:
    ...
    def get_historical_trades(self, days: int = 365, symbol: str = None, start_timestamp_iso: str = None) -> pd.DataFrame:
        try:
            ...
            if start_timestamp_iso:
                query += ' AND timestamp >= ?'
                params.append(start_timestamp_iso)
                # We ignore 'days' if a specific start time is provided
            else:
                start_date_str = (datetime.now() - timedelta(days=days)).isoformat()
                query += ' AND timestamp >= ?'
    ...
    def get_performance_history(self, days: int = 90) -> pd.DataFrame:
        try:
            ...
            df = pd.read_sql_query(query, self.connection, params=[start_date_str])
    ...
    def get_open_stale_trades_by_symbol(self) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            for row in rows:
                trade = dict(row)
                symbol = trade['symbol']
                if symbol not in stale_trades_map:
                    stale_trades_map[symbol] = []
                stale_trades_map[symbol].append(trade)
    ...
    def update_trade_exit_by_id(self, trade_id: int, exit_price: float, pnl_usdt: float, pnl_percent: float, exit_time_iso: str, exit_reason: str) -> bool:
        """
        Updates a trade's exit information using its primary key trade_id.
        """
        try:
            cursor = self.connection.cursor()
            cursor.execute('''
                UPDATE trades
                SET exit_price = ?, pnl_usdt = ?, pnl_percent = ?, exit_reason = ?, 
                    outcome_updated = 1, timestamp = ?
                WHERE id = ? AND exit_price IS NULL
            ''', (exit_price, pnl_usdt, pnl_percent, exit_reason, exit_time_iso, trade_id))
            
            self.connection.commit()
            if cursor.rowcount > 0:
                self.logger.info(f"Reconciled and updated exit for trade with ID {trade_id}")
                return True
            else:
                 self.logger.warning(f"Could not find open trade with ID {trade_id} to reconcile.")
                 return False
        except Exception as e:
            self.logger.error(f"Failed to update trade exit for ID {trade_id}: {e}", exc_info=True)
            if self.connection: self.connection.rollback()
...
if __name__ == "__main__":
    ...
    trade_info = {
        ...
        'pnl_usdt': None,
        'pnl_percent': None,
        'confidence': np.float64(75.0),
        'composite_score': np.float64(25.5),
    ...
    db.store_prediction_quality(**pred_data)
    ...
    training_history = {
        ...
        'final_f1': 0.715,
        'feature_count': 25,
        'parameters_used': {'n_estimators': np.int64(100), 'max_depth': 10},
        'training_bars_used': 1000
    }
